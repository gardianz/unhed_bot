...
def build_lines(markets, state):
    ...
    if state.events:
        event_lines = [Text("Events", style="bold white")]
        for item in state.events[-6:]:
            event_lines.append(Text(item, style="bright_black"))
        blocks.append(Panel(Group(*event_lines), border_style="blue", padding=(0, 1)))
...
def freeze_segment_metrics(state: BotState, market) -> None:
    ...
    freeze_thresholds = (16 * 60, 11 * 60, 6 * 60, 3 * 60)
    for idx, threshold in enumerate(freeze_thresholds):
        if market.timer_left <= threshold and round_state.frozen_segments[idx] is None:
            round_state.frozen_segments[idx] = copy.deepcopy(metrics.segments[idx])

    if round_state.frozen_range_metrics is None and all(round_state.frozen_segments):
        frozen_segments = tuple(round_state.frozen_segments)
        signed_ranges = [segment.signed_range for segment in frozen_segments]
        average_1 = sum(signed_ranges) / 2
        average_2 = average_1 / 3
        width_1 = abs(average_1)
        width_2 = abs(average_2)
        round_state.frozen_range_metrics = copy.deepcopy(metrics)
        round_state.frozen_range_metrics.segments = frozen_segments  # type: ignore[misc]
        round_state.frozen_range_metrics.average_1 = average_1  # type: ignore[misc]
        round_state.frozen_range_metrics.average_2 = average_2  # type: ignore[misc]
        round_state.frozen_range_metrics.range_1_low = market.target_price - width_1  # type: ignore[misc]
        round_state.frozen_range_metrics.range_1_high = market.target_price + width_1  # type: ignore[misc]
        round_state.frozen_range_metrics.range_2_low = market.target_price - width_2  # type: ignore[misc]
        round_state.frozen_range_metrics.range_2_high = market.target_price + width_2  # type: ignore[misc]
